...
# Fetch soil data using hardcoded data
def fetch_soil_data(location):
    print(f"Fetching hardcoded soil data for {location} 🌱")
    state = None
    ...
    if location_lower in CITY_TO_STATE:
        ...
        if not state:
            state = 'default'
            print(f"Using default soil data for unknown location '{location}' ⚠")
        else:
...
def generate_pdf_in_memory(recommendation, user_data, soil_data):
    ...
    try:
        ...
        print("Warning: DejaVuSans.ttf not found. Falling back to Helvetica (using Rs. instead of ₹). 📄⚠")
